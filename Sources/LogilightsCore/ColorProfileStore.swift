...
public struct ColorProfile: Codable {
    public var colorsByModel: [String: LogitechColor] = [:]
}
...
public final class ColorProfileStore: ObservableObject {
    public static let defaultColor = LogitechColor(red: 0x00, green: 0x80, blue: 0xff)

    @Published public private(set) var profile: ColorProfile
    ...
    public func color(for model: LogitechKeyboardModel) -> LogitechColor {
        profile.colorsByModel[model.rawValue] ?? Self.defaultColor
    }

    public func setColor(_ color: LogitechColor, for model: LogitechKeyboardModel) {
